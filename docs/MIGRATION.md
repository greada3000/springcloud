...
| userfollow-service / 8110 | `/api/follows` |
...
- 以业务领域组织 `article`、`circle`、`follow`、`review`、`user` 包。
...
- 用户、圈子和评论表名保持不变；关注关系使用 `tb_user_follow` 表，文章已从 Elasticsearch `article` 索引迁移为 MySQL `tb_article` 表。
- 已存在旧关注表时，执行 `database/migrate_user_follow.sql` 将表和字段迁移为新的 follow 命名。
